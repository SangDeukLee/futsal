...
class Team extends CI_Controller
{
    function __construct()
    {
        ...
        $this->load->model('Teampagereply_model');
    ...
    public function getTeamList()
    {
        ...
        $team['teamList'] = $this->Team_model->getTeamList($user_id);
    ...
    {
        $user_id = isset($_SESSION['loginID']) ? $_SESSION['loginID'] : null;
        $team['team_check'] = $this->Team_model->teamCheck($user_id);
        $this->load->view('./_templates/header.php', $this->data);
        $this->load->view('./_templates/Laside.php', $this->data);
    ...
    public function make()
    {
        if (isset($_POST["submit_make_team"])) {
            ...
            $this->Team_model->makeTeam($team_name, $team_ability, $team_produce, $team_leader, $team_home, $main_team_check);
            ...
            $this->Team_model->updateTeamimg($pfimage, $psimage, $team_name);
        }
    ...
    public function page()
    {
        ...
        $team['teamMember'] = $this->Team_model->teamMemberCheck($_SESSION['loginID'], $team_name);
        $config = array();
        $config["base_url"] = "/team/page/";
        $config['total_rows'] = $this->Team_model->teamBoard_count($team_name);
        $config["per_page"] = 5;
        ...
        $team["team_board"] = $this->Team_model->fetch_boardList($team_name, $config["per_page"], $page);

        $team["links"] = $this->pagination->create_links();

        $team['team_page'] = $this->Team_model->getTeam($team_name);
    ...
    public function write()
    {
        ...
        if (!$teampage_num) {         //일반글
            $autoTeampage_num = $this->Team_model->teampageWrite($team_name, $user_id, $teampage_category, $teampage_title, $teampage_content);
            $teampage_group_num = $autoTeampage_num;
            $teampage_depth = 0;
            $teampage_ord = 0;
            $teampage_num = $autoTeampage_num;
            $this->Team_model->updateTeampageResponse($teampage_group_num, $teampage_depth, $teampage_ord, $teampage_num);
        } else {                      //답글
            $teampage = $this->Team_model->teampageViewByTeampgeNum($teampage_num);

            $teampage_group_num = $teampage->teampage_group_num;
            $teampage_depth = $teampage->teampage_depth + 1;
            $this->Team_model->updateTeampageOrd($teampage_group_num, $teampage->teampage_ord);
            $teampage_ord = $teampage->teampage_ord + 1;
            $this->Team_model->insertResponseBoard($team_name, $user_id, $teampage_category, $teampage_title, $teampage_content, $teampage_group_num, $teampage_depth, $teampage_ord);
    ...
    public function view($teampage_num, $teampage_reply_num, $modifyDistinction)
    {
        $team['team_page'] = $this->Team_model->teampageViewByTeampgeNum($teampage_num);
        $team['team_reply'] = $this->Teampagereply_model->getTeamReplyList($teampage_num);
        ...
        if ($modifyDistinction) {//댓글수정버튼 클릭한거랑 안한거 구별해서 보내주기!
            $team['teampage_reply_num'] = $teampage_reply_num;
            $team['modifyDistinction'] = $modifyDistinction;
    ...
    public function delete($teampage_num)
    {
        ...
        $this->Team_model->teampageDelete($teampage_num);
        $team['teamMember'] = $this->Team_model->teamMemberCheck($_SESSION['loginID'], $team_name);
        $config = array();
    ...
    public function modify()
    {
        if (isset($_POST['submit_teampage_modify'])) {
            ...
            $teampage_title = isset($_REQUEST['teampage_title']) ? ($_REQUEST['teampage_title']) : null;
            $teampage_content = isset($_REQUEST['teampage_content']) ? ($_REQUEST['teampage_content']) : null;
            $this->Team_model->teampageModify($teampage_num, $teampage_title, $teampage_content);
        }
        $this->view($teampage_num, 0, 0);
    }
    ...
    public function teampageReplyWrite()
    { //team페이지 댓글입력
        $user_id = isset($_SESSION['loginID']) ? $_SESSION['loginID'] : null;
        $teampage_num = isset($_REQUEST['teampage_num']) ? ($_REQUEST['teampage_num']) : null;
        $teampage_reply_content = isset($_REQUEST['teampage_reply_content']) ? ($_REQUEST['teampage_reply_content']) : null;

        $this->Teampagereply_model->insertReply($user_id, $teampage_num, $teampage_reply_content);

        $this->view($teampage_num, 0, 0);
    }

    public function teampageReplyModify()
    {
        $teampage_reply_num = isset($_REQUEST['teampage_reply_num']) ? $_REQUEST['teampage_reply_num'] : null;
        $teampage_num = isset($_REQUEST['teampage_num']) ? $_REQUEST['teampage_num'] : null;
        $teampage_reply_content = isset($_REQUEST['teampage_reply_content']) ? $_REQUEST['teampage_reply_content'] : null;

        $this->Teampagereply_model->modifyReply($teampage_reply_num, $teampage_reply_content);
        $this->view($teampage_num, 0, 0);
    }

    public function teampageReplyDelete($teampage_num, $teampage_reply_num)
    {
        $this->Teampagereply_model->deleteReply($teampage_reply_num);
        $this->view($teampage_num, 0, 0);
    }
    ...
    public function teammodify()
    {
        $team_name = isset($_REQUEST['team_name']) ? ($_REQUEST['team_name']) : null;
        $team_ability = isset($_REQUEST['team_ability']) ? ($_REQUEST['team_ability']) : null;
        $team_home = isset($_REQUEST['team_home']) ? $_REQUEST['team_home'] : null;

        if (isset($_POST["teamModify"])) {
            if ($_POST['home'][0]) {
                $team_home = implode("-", $_POST['home']);
            } else {
                $team_home = implode("-", $team_home);
            }

            $this->Team_model->updateTeam($team_name, $team_ability, $team_home);
            if ($_FILES['pfimage']['name']) {
                $team_pfimage = isset($_REQUEST['team_pfimage']) ? ($_REQUEST['team_pfimage']) : null;
                $team_psimage = isset($_REQUEST['team_psimage']) ? ($_REQUEST['team_psimage']) : null;
                $teamImgSavePath = "public/img/team/";
                $thumbnailImgSavePath = "public/img/team_s/";
                $fileMaxSize = 2000000;

                if ($team_name) {
                    unlink($teamImgSavePath . $team_pfimage);
                    unlink($thumbnailImgSavePath . $team_psimage);
                }
    ...
    public function memberlist()
    {
        $team_name = isset($_REQUEST['team_name']) ? ($_REQUEST['team_name']) : null;
        $team['team_page'] = $this->Team_model->getTeam($team_name);
        $team['team_memberlist'] = $this->Team_model->memberList($team_name);
        $this->load->view('./_templates/header.php', $this->data);
        $this->load->view('./_templates/Laside.php', $this->data);
        $this->load->view('team/page', $team);
    ...
    public function teamdelete(){
        ...
        unlink("public/img/team/". $team_pfimage);
        unlink("public/img/team_s/". $team_psimage);
        $this->Team_model->teamdelete($team_name);
        $user_id = isset($_SESSION['loginID']) ? $_SESSION['loginID'] : null;
    ...
    public function kickout($user_id,$attach_team_num){

        $team_name = $this->Team_model->findteamname($attach_team_num);
        $this->Team_model->kickout($user_id,$attach_team_num);
        $team['team_page'] = $this->Team_model->getTeam($team_name->team_name);
        $team['team_memberlist'] = $this->Team_model->memberList($team_name->team_name);
    ...
    public function maketeamcheck()
    {
        $team_name = $this->input->post('team_name');
        $query = $this->Team_model->makeTeamCheck($team_name);
        if ($query) {
            echo "success";
        } else {
            echo "fail";
